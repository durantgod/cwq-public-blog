<template><div><h1 id="rocketmq与kafka选型" tabindex="-1"><a class="header-anchor" href="#rocketmq与kafka选型" aria-hidden="true">#</a> RocketMq与Kafka选型</h1>
<h3 id="rocketmq与kafka选型-1" tabindex="-1"><a class="header-anchor" href="#rocketmq与kafka选型-1" aria-hidden="true">#</a> RocketMq与Kafka选型</h3>
<pre><code>01.两者的相同点
02.部署架构不同
03.工作流程不同
04.日志存储方式不同
...
<p>首先来看看两者的相同之处</p>
<h4 id="_01-两者的相同点" tabindex="-1"><a class="header-anchor" href="#_01-两者的相同点" aria-hidden="true">#</a> 01.两者的相同点</h4>
<pre><code>两者底层原理有很多相似之处，RocketMQ借鉴了Kafka的设计
两者均采用顺序写、零拷贝机制进行写消息与发送消息，极大地保证了系统的性能
</code></pre>
<h4 id="_02-部署架构不同" tabindex="-1"><a class="header-anchor" href="#_02-部署架构不同" aria-hidden="true">#</a> 02.部署架构不同</h4>
<pre><code>Kafka的部署架构
看下面这幅Kafka部署架构图
</code></pre>
<figure><img src="https://oss-cwq.oss-rg-china-mainland.aliyuncs.com/kafkajg.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<pre><code>1）图中，除了包含前面说到的生产者Producer、Kafka集群以及消费者Consumer三个角色之外，还包含了用于存储信息的注册中心-Zookeeper
2）生产者：用于发送消息的客户端
...
<h4 id="_2-rocketmq" tabindex="-1"><a class="header-anchor" href="#_2-rocketmq" aria-hidden="true">#</a> 2 RocketMq</h4>
<figure><img src="https://oss-cwq.oss-rg-china-mainland.aliyuncs.com/rocketmq.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<pre><code>这个是rocketMq的集群架构图，里面也包含了四个主要部分：NameServer集群,Producer集群,Cosumer集群以及Broker集群
...
<h4 id="_03-可靠性不同" tabindex="-1"><a class="header-anchor" href="#_03-可靠性不同" aria-hidden="true">#</a> 03.可靠性不同</h4>
<p>RocketMq分别支持同步异步复制和同步异步刷盘两种机制的配置，针对这两种机制，我们分别来了解一下</p>
<p>1）同步异步复制</p>
<p>同步异步复制区别在于消息发送到master节点，是否会等待向slave节点复制结束再返回</p>
<p>a 同步复制<br>
<img src="https://oss-cwq.oss-rg-china-mainland.aliyuncs.com/tb.png" alt="" loading="lazy"></p>
<p>当生产者将消息发送到broker的master节点时，master会首先将消息复制到slave节点，等待复制动作完成之后，才会给客户端返回“发送成功”的响应，消息可靠性得到保证</p>
<p>b 异步复制<br>
<img src="https://oss-cwq.oss-rg-china-mainland.aliyuncs.com/yb.png" alt="" loading="lazy"></p>
<p>当生产者将消息发送到broker的master节点时，会直接返回一个发送成功的状态响应，并不会等待复制动作的结束，消息可靠性不够高，因为可能会出现网络抖动，导致复制不成功，消费者消费消息不及时的情况</p>
<p>2）同步异步刷盘</p>
<p>同步异步刷盘的区别在于，消息存储在内存（memory）中以后，是否会等待执行完刷盘动作再返回，即是否会等待将消息中的消息写入磁盘中</p>
<p>a 异步刷盘<br>
<img src="https://oss-cwq.oss-rg-china-mainland.aliyuncs.com/ybsp.png" alt="" loading="lazy"></p>
<p>在返回写成功状态时，消息可能只是被写入了内存，并不会等待消息从内存中写入磁盘就返回。所以写操作的返回快，吞吐量大；当内存里的消息量积累到一定程度时，统一触发写磁盘操作，快速写入</p>
<p>b 同步刷盘</p>
<figure><img src="https://oss-cwq.oss-rg-china-mainland.aliyuncs.com/tbsp.png" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>在返回写成功状态时，消息已经被写入磁盘。具体流程是，消息写入内存之后，会立刻通知刷盘线程刷盘，然后等待刷盘完成，刷盘线程执行完成后唤醒等待的线程，返回消息写成功的状态</p>
<p>实际生产中，该如何选择这两种机制的配置呢？权衡性能和可靠性两方面，建议使用异步刷盘，同步复制的形式进行配置，这样即使有一台机器出故障，仍然可以保证数据不丢</p>
<p>而Kafka则只支持异步复制，异步刷盘的机制，虽然在性能上会远远大于RocketMq配置同步复制，同步刷盘的情况，但可靠性会差很多，一旦操作系统出现问题，或者master节点出现故障，数据未及时写入slave节点，数据就很有可能丢失。因此对于一些对数据的可靠性要求比较高的业务场景，可以采用RocketMq而不是Kafka</p>
<h3 id="_04-工作流程不同" tabindex="-1"><a class="header-anchor" href="#_04-工作流程不同" aria-hidden="true">#</a> 04.工作流程不同</h3>
<pre><code>RocketMq的工作流程如下：
